...
{
    public class NettleDeskPlayerController : PlayerController
    {
        public override Vector3 Position
        {
            get => NettleDeskPlayer.NettleDeskPlayerController.Instance ? NettleDeskPlayer.NettleDeskPlayerController.Instance.Position : Vector3.zero;
            set => NettleDeskPlayer.NettleDeskPlayerController.Instance.SetPosition(value);
        }

        public override Quaternion Rotation
        {
            get => NettleDeskPlayer.NettleDeskPlayerController.Instance ? NettleDeskPlayer.NettleDeskPlayerController.Instance.Rotation : Quaternion.identity;
            set => NettleDeskPlayer.NettleDeskPlayerController.Instance.SetRotation(value);
        }

        public NettleDeskPlayerController()
        {
            Tracking = new NettleDeskPlayerTracking();
            RigInitializer = new NettleDeskPlayerRigInitializer();
            Nodes = new NettleDeskPlayerNodes();
            Controls = new NettleDeskPlayerControls();
        }
...
//        }
        ...
        {
            NettleDeskPlayer.NettleDeskPlayerController.Instance.SetRotation(rotation);
        }
        
        private class NettleDeskPlayerTracking : PlayerTracking
        {
        ...
        private class NettleDeskPlayerRigInitializer : PlayerRigInitializer
        {
            public override GameObject InitializeRig()
            {
                return Resources.Load<GameObject>("PlayerRig/NettleDeskPlayer");
            }
        }
        
        private class NettleDeskPlayerNodes : PlayerNodes
        {
            public NettleDeskPlayerNodes()
            {
                Rig = new NettleDeskNode();
                Head = new NettleDeskNode();
                RightHand = new NettleDeskControllerNode();
                LeftHand = RightHand;
            }
            
            public override void Init(GameObject rig)
            {
                var player = rig.GetComponentInChildren<NettleDeskPlayer.NettleDeskPlayerController>();
                Head?.SetNode(player.Head);
                Rig.SetNode(rig);
            }
            ...
            private class NettleDeskNode : Node
            { }
            
            private class NettleDeskControllerNode : ControllerNode
            {
        ...
        private class NettleDeskPlayerControls : PlayerControls
        {
