...
namespace Varwin
{
    ...
    public class VString
    {
        ...
        public static string ConvertToString(dynamic a)
        {
            ...
            try
            {
                if (DynamicCast.IsNumericType(a))
                {
                    result = a.ToString(CultureInfo.InvariantCulture);
                }
                else
                {
                    result = a.ToString();
        ...
        // ReSharper disable once MemberCanBePrivate.Global
        public static string Replace(dynamic sourceString, dynamic oldSubstring, dynamic newSubstring)
        {
#if !NET_STANDARD_2_0
            string convertedSourceString = ConvertToString(sourceString);
            string convertedOldSubstring = ConvertToString(oldSubstring);
            string convertedNewSubstring = ConvertToString(newSubstring);

            return convertedSourceString.Replace(convertedOldSubstring, convertedNewSubstring);
#else
            throw new WrongApiCompatibilityLevelException();
#endif
        }
