    ...
    public class KtxNativeInstance : IMetaData, ILevelInfo
    {
        ...
        unsafe bool Load(NativeSlice<byte> data) {
            ...
            if(status!=KtxErrorCode.KTX_SUCCESS) {
                throw new Exception($"KTX error code {status}");
        ...
        private static int RoundToMultipleOfNumber(int value, int multiplier)
        {
            return multiplier * ((value + multiplier/2) / multiplier);
        }
        ...
        public unsafe Texture2D LoadTextureData(GraphicsFormat gf) {
            ...
            var texture = new Texture2D(
                RoundToMultipleOfNumber((int) baseWidth, 4), // Compression format requirement
                RoundToMultipleOfNumber((int) baseHeight, 4), // Compression format requirement
                gf,
