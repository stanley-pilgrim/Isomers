...
{
    public class PreferencesHelper : MonoBehaviour
    {
        /// <summary>
        ...
        {
            const string preferencesType = "UnityEditor.SettingsWindow";
            ...
            var refreshField = prefType.GetField(refreshPreferencesFieldName,
            ...
            {
                // find method that runs PreferencesWindow and invoke it
                var refreshMethod = prefType.GetMethod(addCustomSectionsMethodName,
                    BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            var selectedProp = prefType.GetProperty(selectedSectionPropertyName,
                BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public static bool CheckIfCacheServerIsOff() => EditorPrefs.GetInt("CacheServerMode") == 2;

        public static void SwitchCacheServerMode(bool turnOn)
        {
            EditorPrefs.SetInt("CacheServerMode", turnOn ? 0 : 2);
            ReloadCacheServerPrefs();
        }

        public static void ReloadCacheServerPrefs()
        {
            ...
            readMethod = typeof(Editor).Assembly.GetType("UnityEditor.CacheServerPreferences")
                .GetMethod("WritePreferences", BindingFlags.Static | BindingFlags.Public);
            readMethod.Invoke(null, null);
        }

        public static void SwitchCacheCustomPathChecker(bool state)
        {
            EditorPrefs.SetBool("LocalCacheServerCustomPath", state);
            ReloadCacheServerPrefs();
        }
    }
}